...
- 先建立 Hungarian GP `race` placeholder（empty `entries`），維持本輪只上線排位資料、不混入 Sunday race outcome
- 驗證 `pnpm build` 通過（R11 Hungarian GP qualifying update 後）
...
- 依 Gore 指示暫時不更新 Belgian GP 正賽相關資料；本輪只處理排位賽資料與 qualifying context
- 驗證 `pnpm build` 通過（R10 Belgian GP qualifying-only update 後）
...
- 新增 British GP `cadillac.driverNotes`：Perez 聚焦 P20 起跑、Lap 22 VSC pit window 與最終 P14；Bottas 聚焦 Cadillac Austria upgrade 首次完整 race distance 與賽後判罰後 P16
- 驗證 `pnpm build` 通過（R9 British GP final race classification update 後）
...
- 修正 Race card / chart meta 顯示：`Race Score`、`Best Cadillac Finish`、Latest 與 tooltip 在雙 DNF 時顯示 `Double DNF`，與真正 `No race data` / `N/A` 區分
- 驗證 `pnpm build` 通過（Race Benchmark double-DNF display fix 後）
...
- 進一步強化 Race trend non-classified marker：`DNF` 改用叉記號、`DNS/DSQ` 改用空心圈，讓底部落點更像狀態事件而不是一般 score 點
- 驗證 `pnpm build` 通過（race chart status marker refinement 後）
